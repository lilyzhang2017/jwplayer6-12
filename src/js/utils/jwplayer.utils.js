(function(jwplayer) {
    ...
    utils.saveCookie = function(name, value) {
        ...
        //console.log(this.isSafari()+'save cookies==='+value)
        document.cookie = 'jwplayer.' + name + '=' + value + '; path=/';
    };

    /** Retrieve saved  player settings 
     Safari浏览器不支持将非ASCII字符存入Cookie，所以中文在保存的时候就会出问题，
     分号(";")也不能存在Cookie中，所以需要通过方法去除内容中的分号，
     在Cookie保存非ASCII字符的时候需要通过UrlEncode / UrlDecode方法来编码和解码问题解决
    ...
    utils.getCookies = function() {
        ...
        for (var i = 0; i < cookies.length; i++) {
            ...
            if (split[0].indexOf('jwplayer.') === 0) {
                if(this.isSafari()){
                    var str=decodeURI(split[1]);//编码入cookie
                    jwCookies[split[0].substring(9, split[0].length)] = str;
                    //jwCookies[split[0].substring(9, split[0].length)] = split[1];  
                    //console.log(str+'===getCookies-----safari='+this.isSafari());
                }else{
                    jwCookies[split[0].substring(9, split[0].length)] = split[1]; 
                   // console.log(split[1]+'===getCookies-----safari='+this.isSafari()); 
                }
               //console.log(str+'===getCookies-----safari='+this.isSafari());
            }
        }
