(function(jwplayer) {
    ...
    function VideoProvider(_playerId) {
        ...
        function _onClickHandler() {
            if(utils.isMSIE(10)){
                //避免Ie10单击全屏问题；
               // console.log('ie10======='+utils.isMSIE(10))
            }else{
        ...
        this.play = function() {
            ...
            var playbackspeed=window.thisCurrentRate();
           //console.log('play----------video---------'+playbackspeed);
            if(utils.isIE()){
               // jwplayer().seek(jwplayer().getPosition());
               // jwplayer().onSeek(function(){_videotag.playbackRate = playbackspeed;});
                jwplayer().onPause(function(){_videotag.playbackRate = playbackspeed;});
                jwplayer().onPlay(function(){_videotag.playbackRate = playbackspeed;});
                
            } 
            _videotag.playbackRate =Number(playbackspeed);
            // console.log(_videotag.playbackRate+"==speedRate-*-*-*-isie="+utils.isIE())
            //
            if (_attached && !_dragging) {
