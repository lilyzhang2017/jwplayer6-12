package com.longtailvideo.jwplayer.player {
	...
	import com.longtailvideo.jwplayer.player.imoocAddClass.ErrorWindow;
	...
	public class JavascriptAPI {
		...
		private static function setupJSListeners():void {
			try {
				...
				//zhangli add---------showErrorWin()
				
				ExternalInterface.addCallback("flash_showErrorWin",js_showErrorWin);
				ExternalInterface.addCallback("flash_hideErrorWin",js_hideErrorWin);
			} catch(e:Error) {
		...
		private static function js_fullscreen(fullscreenstate:*=null):void {
			...
			if (String(fullscreenstate).toLowerCase() == "true") {
				...
				//_player.fullscreen(true);js 不可以控制flash 模式全屏功能；
				//ExternalInterface.call("flashToJs","full true");
				return;
			} else {
		...
		private  static var showError:ErrorWindow;
		private static function js_showErrorWin():void{
			showError=ErrorWindow.getInstance();
		
			showError.showErrorWin();
		}
		
		private static function js_hideErrorWin():void{
		
			showError.hideErrorwin();
		}
	}
}
