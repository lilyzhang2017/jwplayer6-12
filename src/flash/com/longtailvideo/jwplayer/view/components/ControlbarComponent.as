package com.longtailvideo.jwplayer.view.components {
	...
	public class ControlbarComponent extends CoreComponent implements IControlbarComponent {
		...
		//protected var _defaultLayout:String = "[play prev next elapsed][time alt][duration mute volumeH  hd cc set track  cast fullscreen]";
		protected var _defaultLayout:String = "[play prev next elapsed][time alt][duration mute volumeV  hd cc set track  cast fullscreen]";
        ...
        public    var _timeSlider:TimeSlider;
		...
		import com.longtailvideo.jwplayer.view.components.ToolSetOverlay;
		...
		public function bigTimeSlider():void{
			//bigControlbar();
			 timeMaskMc.y=-10;
			_timeSlider.thumbVisible=true;
			
		}
		public function smallTimeSlider():void{
			if(_timeSlider._dragging){
				return;
			}
				...
				case MediaEvent.JWPLAYER_MEDIA_TIME:
                    ...
                    if (isLive && evt.type == MediaEvent.JWPLAYER_MEDIA_TIME) {
						if (!_instreamMode) {
                            ...
                            //   is a live broadcast to error
                            var backup:String = (_casting ? "" : "error");
        ...
        private function updatePositionAndDurationText():void {
            ...
            if (isSmallPlayer) {
				...
				elapsedString = Strings.digits(_lastPos)+"/"+durationString;
				//var str:String=elapsedString+"/"+durationString;
			}
            ...
            if (durationTextField) {
				...
				durationTextField.visible=false;
			}
		...
		private function setupDefaultButtons():void {
			...
			if(_player.config.showset==true){//显示set  show set  button
				addComponentButton('set', null);
			}
			...
			addSlider('volumeV', ViewEvent.JWPLAYER_VIEW_CLICK, volumeHandler, false);
			_volSliderV = getSlider('volumeV');
//			var slidermask:Sprite=new Sprite();
//			slidermask.graphics.beginFill(0x14191e,0.9);
//			slidermask.graphics.drawRect(0, 0, 20, 20);
//			slidermask.graphics.endFill();
//			this.addChild(slidermask);
//			slidermask.x=RootReference.stage.stageWidth-183;
//			slidermask.y=_volSliderV.y+5;
			
			if (_buttons.hd) {
				_buttons.hd.addEventListener(MouseEvent.MOUSE_OVER, showHdOverlay);
				_buttons.hd.addEventListener(MouseEvent.MOUSE_OUT, setHdTxtColor);
			}
			if (_buttons.cc) {
				_buttons.cc.addEventListener(MouseEvent.MOUSE_OVER, showCcOverlay);
			}
			if (_buttons.set) {
				_buttons.set.addEventListener(MouseEvent.MOUSE_OVER, showSetOverlay);
			}
			if (_buttons.track) {
		...
		private function setupOverlays():void {
			...
			_hdOverlay.y=RootReference.stage.stageHeight-175;
			...
			_setOverlay=new ToolSetOverlay(_player.config.ish5,_player.config.shownextlevel);
			_setOverlay.name="setOverlay";
			createOverlay(_setOverlay, _buttons.set);
			...
			if (_volSliderV) {
				...
				_volumeOverlay.y=RootReference.stage.stageHeight-(50+_setOverlay.height);
			}

		}

		//set over lay--------------------------
		private var _setOverlay:ToolSetOverlay;

		
		private function showSetOverlay(e:MouseEvent):void{
			if(_timeSlider._dragging){
				return;
			}
			...
			hideHdOverlay();
			_setOverlay.show();
			_setOverlay.x=RootReference.stage.stageWidth-270;
			
			_setOverlay.y=RootReference.stage.stageHeight-190;
			
			
		}
		private function createOverlay(overlay:MovieClip, button:DisplayObject):void {
			if (button && overlay) {
				...
				if(overlay.name=="hdOverlay"){
					overlay.y=RootReference.stage.stageHeight-175;
				}
			}
		}
		...
		private function showHdOverlay(evt:MouseEvent):void {
			...
			if (_audioMode) return;
			if (_hdOverlay && _levels && _levels.length > 2){
				_hdOverlay.show();
				_hdOverlay.y=RootReference.stage.stageHeight-50;
			}
			hideCcOverlay();
			hideVolumeOverlay();
			hideTrackOverlay();
			hideSetOverlay();
			//hd 0xf01400
			var hd=evt.currentTarget;
			
			
			var txt:TextField=hd.getChildAt(1);
			
			txt.textColor= 0xf01400;
		}
		private function setHdTxtColor(e:MouseEvent):void{
			var hd=e.currentTarget;
		    var txt:TextField=hd.getChildAt(1);
			txt.textColor= 0x787d82;
		}
		...
		private function hideSetOverlay(evt:MouseEvent=null):void {
			if (_setOverlay && !evt) {
				_setOverlay.hide();
			}
		}
		...
		private function setHDIndex():void
		{
			//_currentQuality
			
			var btn=getButton("hd");
			for(var i:int=0;i<btn.numChildren;i++)
			{
				var child=btn.getChildAt(i);//zhanglili
				
				if(child.name=="hd" && child is TextField)
				{
					
					child.text=_levels[_currentQuality].label;
					break;
				}
			}
		...
		private function addComponentButton(name:String, event:String, eventData:*=null):void {
			...
			if(name=="hd"){/// zhangli-hd 创建文本框
				
				var txt:TextField=new TextField()
				var format:TextFormat = new TextFormat();
				format.font = "Microsoft Yahei";
				format.color = 0x787d82;
				format.size = 12;
				txt.width=28;
				txt.height=18;
				//txt.x=-25;
				//txt.y=3;
				txt.x = Math.abs(button.width - txt.width) / 2;
				txt.y = Math.abs(button.height - txt.height) / 2;
				//txt.border=true;
		...
		public function drawMask(slider:Slider):void
		{
			
			timeMaskMc= new Sprite();
			timeMaskMc.graphics.beginFill(0x0099ff,1);
			timeMaskMc.graphics.drawRect(0, 0, RootReference.stage.stageWidth, 16);
			timeMaskMc.graphics.endFill();
			this.addChild(timeMaskMc);
			timeMaskMc.x=0;
			timeMaskMc.y=0;
			slider.mask=timeMaskMc;
		...
		private function forward(evt:ViewEvent):void {
			//if范围内是新添加的内容  zhangli add ,修改 拖动音量，静音切换问题
			if(evt.type=="jwplayerViewMute" && _player.config.volume==0)
			{
				_player.mute(true);
				var volumeEvent:MediaEvent = new MediaEvent(MediaEvent.JWPLAYER_MEDIA_VOLUME);
				volumeEvent.volume = 50;
				_player.config.volume=50;
				_player.config.mute=true;
		...
		//键盘控制显示音量控制栏
		public function showVolumeOverlay2():void {
			if (_audioMode || _instreamMode) return;
			if (_volumeOverlay) _volumeOverlay.show();
			hideHdOverlay();
			hideSetOverlay();
		...
		public function hideVolumeOverlay2():void {
			//if (_volumeOverlay && !evt) {
				_volumeOverlay.hide();
			//}
		}
		public function hideVolumeOverlay(evt:MouseEvent=null):void {
			if (_volumeOverlay && !evt) {
				_volumeOverlay.hide();
			}
		}
		...
		private var isTweenBar:Boolean=false;
		public var isBig:Boolean=false;
		public function bigControlbar():void{
			if(!isTweenBar && !isBig){
				isTweenBar=true;
				var h=RootReference.stage.stageHeight;
				this.y=h-5;
				TweenLite.to(this,0.3,{y:h-40,onComplete:barTweenFinish});
				isBig=true;
			}
			
		}
		public function smallControlbar():void{
			if(_timeSlider._dragging){
				return;
			}
			//if (!_audioMode) {
				...
				if(!isTweenBar){
					...
					var h=RootReference.stage.stageHeight;
					this.y=h-40;
					TweenLite.to(this,0.3,{y:h-5,onComplete:barTweenFinish});
					isBig=false;
					smallTimeSlider();
				}
			//}
		...
		public function controlbarIsBig():Boolean{
			if(isBig){
				return true;
			}else{
				return false;
		...
		private function barTweenFinish():void{
