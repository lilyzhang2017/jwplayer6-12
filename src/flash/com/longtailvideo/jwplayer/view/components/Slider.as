...
//			if(_name=="volume"){
//				_progress.width=60;
//				ExternalInterface.call("flashToJs",_progress.width)
//			}
			...
			if(name=="timeSlider"){
				...
				slidermask.graphics.beginFill(0x14191e,1);
				slidermask.graphics.drawRect(0, 0, 78, 40);
				slidermask.graphics.endFill();
				addChild(slidermask);
				slidermask.x=this.x;
				slidermask.y=this.y;
				this.mask=slidermask;*/
				//_progress.width=75;
			}
...
//			}
			...
			if (element) {
				if (_width && _height) {
					...
					if (_vertical) {
						...
						if(element.name=="volumeProgress "){//
		...
		protected function sliderPercent(pixels:Number):Number {
			...
			var n:Number=Math.max(Math.min(1, percent), 0);
			if( _name != "timeSlider"){
				ExternalInterface.call("flashToJs",n);
